...
NOINLINE(double Pss_st::disconnect(std::size_t N))
{
    return Benchmark<Signal, Pss_st>::disconnect(N);
}
NOINLINE(double Pss_st::reconnect(std::size_t N))
{
    return Benchmark<Signal, Pss_st>::reconnect(N);
}
