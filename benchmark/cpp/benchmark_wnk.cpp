...
NOINLINE(double Wnk::disconnect(std::size_t N))
{
    return Benchmark<Signal, Wnk>::disconnect(N);
}
NOINLINE(double Wnk::reconnect(std::size_t N))
{
    return Benchmark<Signal, Wnk>::reconnect(N);
}
