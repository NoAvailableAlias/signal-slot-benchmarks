...
NOINLINE(double Cps::disconnect(std::size_t N))
{
    return Benchmark<Signal, Cps>::disconnect(N);
}
NOINLINE(double Cps::reconnect(std::size_t N))
{
    return Benchmark<Signal, Cps>::reconnect(N);
}
