...
NOINLINE(double Cps_st::disconnect(std::size_t N))
{
    return Benchmark<Signal, Cps_st>::disconnect(N);
}
NOINLINE(double Cps_st::reconnect(std::size_t N))
{
    return Benchmark<Signal, Cps_st>::reconnect(N);
}
