...
NOINLINE(double Lss::disconnect(std::size_t N))
{
    return Benchmark<Signal, Lss>::disconnect(N);
}
NOINLINE(double Lss::reconnect(std::size_t N))
{
    return Benchmark<Signal, Lss>::reconnect(N);
}
